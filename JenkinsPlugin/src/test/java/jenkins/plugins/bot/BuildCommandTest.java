...
public class BuildCommandTest {
    ...
    public void testBuildAll() {
    	...
    	projects.add(mockProject(jobProvider,"proj1"));
    	projects.add(mockProject(jobProvider,"proj2"));
        ...
        JBotSender sender = new JBotSender("sender");
        
        String res = cmd.getReply(bot,sender,new String[]{"build","-all"});
        String[] lines = res.split("\n");
        assertEquals(lines[0],"Start to schedule all projects:");
        assertTrue(lines[1].startsWith("sender: job proj1 build scheduled"));
        assertTrue(lines[2].startsWith("sender: job proj2 build scheduled"));
        assertEquals(lines[3],"All projects have benn scheduled!");
    }
    ...
    private AbstractProject<?, ?> mockProject(JobProvider jobProvider, String name) {
        ...
        when(project.scheduleBuild( anyInt(), any(JBotCause.class) )).thenReturn(true);
        return project;
    }
